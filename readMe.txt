Assignment 3: A Website for Advertisement

Team members: Noor Ul Zain 2528644 Shemin Samiei 2542389

Version of Python used: 3.9

Operating System used: Windows

Team work division:
We divided the work as follows

Database Creation Script                   Shemin
Login, Logout and Session Management       Shemin
Register 				                   Shemin
Access user advertisements 		           Shemin
Deactivate advertisements 		           Noor
Manage Profile                             Noor
Search advertisements and list them categorised by category  Noor
See the details of the selected category                     Noor
Navigation within the website				     Shemin and Noor
Form validation with JavaScript 			     Noor
Design with CSS 					             Shemin

We made sure to do version control and regular updates using Github.
We communicated through whatsapp and also made sure to meet face to face to debug the code
together and test different use cases together. We also reviewed each other's work and gave
suggestions where needed and helped to fix each other's bugs. 